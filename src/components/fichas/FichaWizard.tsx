...
export default function FichaWizard({
  ...
  return (
    <div className="mx-auto max-w-3xl p-6">
      {editando ? (
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-sky-500/20 bg-sky-500/5 px-4 py-2.5">
      ...
      {borradorRecuperado && (
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2 rounded-xl bg-amber-500/10 px-4 py-2.5 text-sm text-amber-400">
          ...
          <button
            ...
            className="rounded-full border border-amber-500/30 px-2.5 py-1 text-xs font-medium transition hover:bg-amber-500/10"
          >
                ...
                return (
                  <label
                    ...
                    className={`flex cursor-pointer items-center justify-between rounded-xl border p-3 transition ${
                      checked ? 'border-orange-400 bg-orange-500/5' : 'border-white/10'
                    }`}
                  >
      ...
      {step === 4 && template && (
        <div>
          ...
          {error && (
            <p className="mb-3 rounded-lg bg-red-500/10 p-2 text-sm text-red-400">
              {error}
            </p>
          )}
    ...
    <div>
      ...
      {campo.imagen_referencia_url && (
        ...
        <img
          ...
          className="mb-2 max-h-56 w-full rounded-lg border border-white/10 object-contain"
        />
      )}
      ...
      {resultado !== null && (
        <>
          ...
          <button
            ...
            className="rounded-full bg-orange-500 px-2 py-1 font-medium text-black transition hover:bg-orange-400"
          >
            Usar
          </button>
        </>
...
function ProductorBanner({
  ...
  const colores: Record<string, string> = {
    organico: 'bg-green-500/10 text-green-400',
        ...
        <div className="mt-2 border-t border-orange-500/10 pt-2">
          ...
          <div className="mt-1 flex flex-wrap gap-1.5">
            {productor.plantas_entregadas.map((e, i) => (
              ...
              >
                {e.cantidad} {e.especie} <span className="text-green-500">({e.anio})</span>
              </span>
            ))}
          </div>
        </div>
