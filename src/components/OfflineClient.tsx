...
export default function OfflineClient() {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-6 text-center">
      ...
      <h1 className="text-lg font-normal text-white">Estás sin conexión</h1>
      ...
      <div className="mt-6 flex w-full max-w-xs flex-col gap-2">
        ...
        <button
          ...
          className="rounded-full border border-white/10 bg-black/40 backdrop-blur-xl px-4 py-2.5 text-sm font-medium text-white transition hover:border-orange-500/40"
        >
          Nueva bitácora
        </button>
        <button
      ...
      {pendientes.length > 0 && (
        <div className="mt-6 w-full max-w-sm rounded-xl border border-white/10 bg-black/40 backdrop-blur-xl p-3 text-left">
          <p className="mb-1.5 font-mono text-[11px] uppercase tracking-wide text-silver">
            {pendientes.length} por subir
          </p>
          <ul className="space-y-1">
            {pendientes.map((p, i) => (
              <li key={i} className="flex items-baseline gap-2 text-xs">
                <span className="shrink-0 rounded bg-white/5 px-1.5 py-0.5 font-medium text-silver">
                  {p.tipo}
                </span>
                <span className="truncate text-silver">{p.etiqueta}</span>
              </li>
            ))}
          </ul>
      ...
      <p className="mt-6 max-w-sm text-xs text-silver">
  ...
  return (
    <div className="flex h-screen w-screen flex-col overflow-hidden">
      <div className="flex items-center gap-3 border-b border-white/10 bg-black/40 px-4 py-2.5 backdrop-blur-xl">
        <button
          onClick={onVolver}
          className="rounded-full px-2 py-1 text-sm text-silver hover:text-white"
        >
          ← Atrás
        </button>
...
function SinDatos() {
  return (
    <div className="mx-auto max-w-md p-10 text-center text-sm text-silver">
...
function BitacoraOffline({ onGuardada }: { onGuardada: () => void }) {
  ...
  if (est.fase === 'cargando')
    return <div className="p-10 text-center text-sm text-silver">Cargando parcelas…</div>
